...
$jscomp.polyfill = function(a, b, d, g) { b && ($jscomp.ISOLATE_POLYFILLS ? $jscomp.polyfillIsolated(a, b, d, g) : $jscomp.polyfillUnisolated(a, b, d, g)) };
...
$jscomp.polyfillIsolated = function(a, b, d, g) {
    ...
    b = b(d);
    null != b && (a ? $jscomp.defineProperty($jscomp.polyfills, e, { configurable: !0, writable: !0, value: b }) : b !== d && (void 0 === $jscomp.propertyToPolyfillSymbol[e] && ($jscomp.propertyToPolyfillSymbol[e] = $jscomp.IS_SYMBOL_NATIVE ? $jscomp.global.Symbol(e) :
...
$jscomp.asyncExecutePromiseGenerator = function(a) {
    function b(g) { return a.next(g) }

    function d(g) { return a["throw"](g) }
    return new Promise(function(g, e) {
        function l(c) { c.done ? g(c.value) : Promise.resolve(c.value).then(b, d).then(l, e) }
        l(a.next())
    })
};
...
$jscomp.polyfill("Symbol", function(a) {
    ...
    var d = 0,
        g = function(e) { if (this instanceof g) throw new TypeError("Symbol is not a constructor"); return new b("jscomp_symbol_" + (e || "") + "_" + d++, e) };
    return g
}, "es6", "es3");
...
$jscomp.polyfill("Promise", function(a) {
    ...
    function d(c) { return c instanceof e ? c : new e(function(f, h) { f(c) }) }
    ...
    b.prototype.asyncExecute = function(c) {
        if (null == this.batch_) {
            this.batch_ = [];
            var f = this;
            this.asyncExecuteFunction(function() { f.executeBatch_() })
        }
        this.batch_.push(c)
    };
    var g = $jscomp.global.setTimeout;
    b.prototype.asyncExecuteFunction = function(c) { g(c, 0) };
    ...
    var e = function(c) {
        ...
        try { c(f.resolve, f.reject) } catch (h) { f.reject(h) }
    };
    e.prototype.createResolveAndReject_ = function() {
        function c(k) { return function(m) { h || (h = !0, k.call(f, m)) } }
        var f = this,
            h = !1;
    ...
    e.prototype.resolveToNonPromiseObj_ = function(c) { var f = void 0; try { f = c.then } catch (h) { this.reject_(h); return } "function" == typeof f ? this.settleSameAsThenable_(f, c) : this.fulfill_(c) };
    ...
    e.prototype.settle_ = function(c, f) {
        if (0 != this.state_) throw Error("Cannot settle(" + c + ", " + f + "): Promise already settled in state" + this.state_);
        this.state_ = c;
        this.result_ = f;
    ...
    e.prototype.scheduleUnhandledRejectionCheck_ = function() {
        var c = this;
        g(function() { if (c.notifyUnhandledRejection_()) { var f = $jscomp.global.console; "undefined" !== typeof f && f.error(c.result_) } }, 1)
    };
    e.prototype.notifyUnhandledRejection_ = function() {
        if (this.isRejectionHandled_) return !1;
        var c = $jscomp.global.CustomEvent,
            f = $jscomp.global.Event,
            h = $jscomp.global.dispatchEvent;
        if ("undefined" === typeof h) return !0;
        "function" === typeof c ? c = new c("unhandledrejection", { cancelable: !0 }) :
            "function" === typeof f ? c = new f("unhandledrejection", { cancelable: !0 }) : (c = $jscomp.global.document.createEvent("CustomEvent"), c.initCustomEvent("unhandledrejection", !1, !0, c));
        c.promise = this;
    ...
    e.prototype.settleSameAsThenable_ = function(c, f) { var h = this.createResolveAndReject_(); try { c.call(f, h.resolve, h.reject) } catch (k) { h.reject(k) } };
    ...
    e.resolve = d;
    e.reject = function(c) { return new e(function(f, h) { h(c) }) };
    e.race = function(c) { return new e(function(f, h) { for (var k = $jscomp.makeIterator(c), m = k.next(); !m.done; m = k.next()) d(m.value).callWhenSettled_(f, h) }) };
    e.all = function(c) {
        var f = $jscomp.makeIterator(c),
            h = f.next();
        return h.done ? d([]) : new e(function(k,
            m) {
            function t(q) {
                return function(r) {
                    n[q] = r;
            ...
            var n = [],
                p = 0;
            do n.push(void 0), p++, d(h.value).callWhenSettled_(t(n.length - 1), m), h = f.next(); while (!h.done)
        })
    };
    return e
}, "es6", "es3");
var network, curnet, mainnet, mainnetAlt, testnet, mainnetname, testnetname, wrongnet, contractAddress, abi, contract, contractSign, tracker, myAddress, signer, provider;
$(function() {
...
function getNetwork() {
    return $jscomp.asyncExecutePromiseGeneratorProgram(function(a) {
        ...
        curnet == mainnet || curnet == mainnetAlt ? (contractAddress = contractAddressMain, $("#curnet").html(mainnetname), tracker = trackerMain, getAddress(), init()) : curnet == testnet ? (contractAddress = contractAddressTest, $("#curnet").html(testnetname), tracker = trackerTest, getAddress(), init()) : ($("#curnet").html(wrongnet), $("#myaddr").html(""),
            $("#buildbtn").prop("disabled", !0), $("#strHtml").html(""));
        a.jumpToEnd()
    })
}
...
function getAddress() {
    ...
    return $jscomp.asyncExecutePromiseGeneratorProgram(function(b) {
        switch (b.nextAddress) {
            ...
            case 3:
                $("#myaddr").html(myAddress), myAddress ? ($("#buildbtn").prop("disabled", !1), $("#errors").html("")) :
                    ($("#buildbtn").prop("disabled", !0), $("#errors").html("Please connect to your wallet!")), b.jumpToEnd()
        }
    })
};